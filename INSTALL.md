...
3. When it asks for your sync folder, type the folder's path (or press **Enter** to use `~/SyncDrop`). If the folder doesn't exist, say **Y** to create it.
   Next it asks what **name** to show in the right-click menu for that folder (Enter keeps "Main Sync"). Then it asks whether you want to add more folders: answer **y**, say how many (1 to 6), and it walks you through each one (path, then menu name).
